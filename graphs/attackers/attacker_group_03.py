...
def _project_box_l2_signed(delta, eps, x2d):
    """
    Projection sûre dans l'ensemble admissible défini par :
    - -x <= delta <= 1 - x   (pour garantir x + delta dans [0, 1])
    - ||delta||_2 <= eps

    Comme x in [0, 1], cette boîte implique automatiquement delta in [-1, 1].
    """
    lower = -x2d
    upper = 1.0 - x2d

    delta = torch.maximum(delta, lower)
...
def _random_signed_start(x2d, eps):
    lower = -x2d
    upper = 1.0 - x2d
    delta = lower + torch.rand_like(x2d) * (upper - lower)
    return _project_box_l2_signed(delta, eps, x2d)


def _finalize_delta(x2d, delta, eps):
    """
    Sécurise la sortie pour respecter strictement les consignes.
    """
    delta = _project_box_l2_signed(delta, eps, x2d)

    # garde-fou numérique : on reconstruit delta depuis une image recadrée
...
def _optimize_phase(
    ...
    """
    Petite attaque PGD-L2 signée avec momentum.
    target_idx = None   -> non ciblée
    target_idx = entier -> ciblée
    """
    if random_start:
        delta = _random_signed_start(x2d, eps)
    else:
    ...
    for k in range(n_steps):
        ...
        with torch.no_grad():
            ...
            if int(y_adv.item()) != int(y_ref.item()):
                return _finalize_delta(x2d, delta, eps), True

    best_delta = _finalize_delta(x2d, best_delta, eps)
    return best_delta, _successful(model, mode, x2d, best_delta, y_ref)
...
def attack(x, f_string, eps):
    ...
    if success:
        _sync_if_needed(device)
        return _finalize_delta(x2d, best_delta, eps).cpu()

    # Phase 2 : non ciblée, départ aléatoire signé
    if _elapsed(start_time, device) <= max_time:
        delta, success = _optimize_phase(
            model=model,
            mode=mode,
    ...
    if _elapsed(start_time, device) <= max_time:
        ...
        for t in targets:
            ...
            if success:
                _sync_if_needed(device)
                return _finalize_delta(x2d, delta, eps).cpu()
